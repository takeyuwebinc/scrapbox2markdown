RSpec.describe Scrapbox2markdown::Converters::CodeBlock do
  describe '#convert!' do
    context 'When passing a multiple line' do
      ...
      before do
        converter = Scrapbox2markdown::Converters::CodeBlock.new(lines)
        converter.convert!
      end

      # マークダウン形式のコードブロックに変換するが、最後の閉じる```は挿入しない
      # Scrapbox2markdown::Converter#code_blockにて挿入する
      expected = [
          "```サンプル.html",
          "<link href=\"https://scrapbox2markdown/test.min.css\" rel=\"stylesheet\">",
          "<script src=\"https://scrapbox2markdown/test.min.js\"></script>",
