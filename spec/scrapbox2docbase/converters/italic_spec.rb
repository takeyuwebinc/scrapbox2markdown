RSpec.describe Scrapbox2docbase::Converters::Italic do
  describe '#convert!' do
    context 'When passing a single line including a multiple italic' do
      subject(:line) { ['[/ First] and [/ Second]'] }
      before do
        converter = Scrapbox2docbase::Converters::Italic.new(line)
        converter.convert!
      end
    ...
    context 'When passing italic and italic with heading' do
      ...
      before do
        converter = Scrapbox2docbase::Converters::Italic.new(lines)
        converter.convert!
      end
