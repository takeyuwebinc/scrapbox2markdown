RSpec.describe Scrapbox2docbase::Converters::List do
  describe '#convert!' do
    context 'When passing a single line starting with a white space' do
      subject(:line) { [' List'] }
      before do
        converter = Scrapbox2docbase::Converters::List.new(line)
        converter.convert!
      end
    ...
    context 'When passing a multiple lines starting with a white space' do
      ...
      before do
        converter = Scrapbox2docbase::Converters::List.new(lines)
        converter.convert!
      end
