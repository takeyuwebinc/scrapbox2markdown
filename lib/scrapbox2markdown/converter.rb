module Scrapbox2markdown
  class Converter
    ...
    def convert!
      ...
      hash = Scrapbox2markdown::Detector.new(@lines).line_numbers_of_others
      others = hash[:others].map { |number| @lines[number] }
      unless others.empty?
        Scrapbox2markdown::Converters::Heading.new(others).convert!
        Scrapbox2markdown::Converters::Emphasis.new(others).convert!
        Scrapbox2markdown::Converters::Italic.new(others).convert!
        Scrapbox2markdown::Converters::Strikethrough.new(others).convert!
        Scrapbox2markdown::Converters::List.new(others).convert!
        # 最後にリンクの除去を行う
        Scrapbox2markdown::Converters::Link.new(others).convert!
      end
      ...
      while has_code_block?
        ...
        # TODO: ここはhashで返す必要はないのでは
        hash = Scrapbox2markdown::Detector.new(@lines).line_numbers_of_code_block
        code_block = hash[:code_block].map { |number| @lines[number] }
        Scrapbox2markdown::Converters::CodeBlock.new(code_block).convert!
      ...
      while has_table?
        ...
        # TODO: ここはhashで返す必要はないのでは
        hash = Scrapbox2markdown::Detector.new(@lines).line_numbers_of_table
        table = hash[:table].map { |number| @lines[number] }
        Scrapbox2markdown::Converters::Table.new(table).convert!
